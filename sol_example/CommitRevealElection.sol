...
contract CommitRevealElection {
    ...
    function commitVote(bytes32 _voteCommit) onlyBefore(commitPhaseEndTime) {
        //        if (now > commitPhaseEndTime) throw; // Only allow commits during committing period  改为用 modifier
    ...
    function revealVote(string _vote, bytes32 _voteCommit) onlyAfter(commitPhaseEndTime) {
        //        if (now < commitPhaseEndTime) throw;   改为用 modifier
        // Only reveal votes after committing period is over
